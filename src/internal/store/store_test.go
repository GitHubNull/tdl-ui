...
func TestInsertAndLoadTask(t *testing.T) {
	...
	if err := s.InsertTask(task, items); err != nil {
...
func TestFileUpsertAndFinish(t *testing.T) {
	...
	tmp := "/tmp/a.mp4.tmp"
	if err := s.UpsertFile(File{TaskID: "t1", Name: "a.mp4", Path: tmp, Size: 100, State: "downloading"}); err != nil {
		t.Fatal(err)
	}
	// 同路径覆盖
	if err := s.UpsertFile(File{TaskID: "t1", Name: "a.mp4", Path: tmp, Size: 200, State: "downloading"}); err != nil {
		t.Fatal(err)
	}
	files, _ := s.ListFiles("t1")
	if len(files) != 1 || files[0].Size != 200 {
		t.Fatalf("同路径应覆盖为 1 行 size=200，实际 %+v", files)
	}

	final := "/tmp/a.mp4"
	if err := s.FinishFile("t1", tmp, File{TaskID: "t1", Name: "a.mp4", Path: final, Size: 200, State: "done"}); err != nil {
		t.Fatal(err)
	}
	files, _ = s.ListFiles("t1")
...
func TestResumePoints(t *testing.T) {
	...
	if err := s.SaveFinished("t1", set); err != nil {
		t.Fatal(err)
	}
	got, err := s.LoadFinished("t1")
	if err != nil || len(got) != 2 {
	...
	if err := s.DeleteResume("t1"); err != nil {
		t.Fatal(err)
	}
	got, _ = s.LoadFinished("t1")
	if len(got) != 0 {
...
func TestCascadeDelete(t *testing.T) {
	s := newTestStore(t)
	if err := s.InsertTask(Task{ID: "t1", Dir: "/tmp", Status: "done"},
		[]Item{{ItemType: ItemTypeURL, URL: "u1"}}); err != nil {
		t.Fatal(err)
	}
	_ = s.UpsertFile(File{TaskID: "t1", Name: "f", Path: "/tmp/f", State: "done"})
	_ = s.SaveFinished("t1", map[string]struct{}{"1:1": {}})

	if err := s.DeleteTask("t1"); err != nil {
		t.Fatal(err)
	}
	if items, _ := s.ListItems("t1"); len(items) != 0 {
		t.Fatal("级联删除应清空消息项")
	}
	if files, _ := s.ListFiles("t1"); len(files) != 0 {
		t.Fatal("级联删除应清空文件")
	}
	if fin, _ := s.LoadFinished("t1"); len(fin) != 0 {
		t.Fatal("级联删除应清空断点")
	}
}
...
func TestImportLegacyJSON(t *testing.T) {
	...
	if err := s.ImportLegacyJSON(jsonPath); err != nil {
		t.Fatalf("导入失败: %v", err)
	}

	tasks, _ := s.LoadAllTasks()
