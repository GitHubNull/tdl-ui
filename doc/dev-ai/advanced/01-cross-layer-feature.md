...
- 评估是否需要新服务，还是挂在现有四服务之一（原则：登录相关 → Auth，任务型长操作 → 新建独立服务或挂 Download，纯配置 → Settings）
...
- 新服务需在 `src/main.go` 的 `Bind` 数组注册，并在 `OnStartup` 链路中完成依赖注入（参考现有四服务的构造顺序：config → emitter → kv → services）
...
```
src/frontend/src/pages/ForwardPage.vue + router.ts + App.vue 导航
```
