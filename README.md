...
- **对话浏览**：双面板浏览对话列表与媒体文件，支持搜索/过滤/分页，缩略图预览，虚拟滚动与智能预加载
- **媒体下载**：从对话勾选选集下载，或粘贴消息链接批量下载，多线程 + 断点续传（与 tdl CLI 同源引擎）
- **任务管理**：实时进度、速度展示，支持暂停 / 恢复（断点续传）/ 取消；单文件操作（重新下载 / 打开目录 / 删除记录 / 删除文件）
- **媒体预览**：Lightbox 全屏预览（图片缩放 / 平移），自定义视频播放器（边下边播、键盘快捷键）
- **脚本引擎**（Yaegi，Go 语法）：
  - `Filter` / `Rename` —— 按条件跳过文件、自定义文件名
  - `OnTaskStart` / `OnFileDone` / `OnTaskDone` —— 任务生命周期钩子
  - 内置脚本模板、启用开关、校验与试运行
- **日志系统**：实时日志浏览、历史文件查看、搜索过滤、日志导出（.log / .txt / .csv）
- **代理支持**：SOCKS5 / HTTP 代理
...
4. 在 **下载** 页查看实时进度，可暂停 / 恢复（断点续传）/ 取消；展开文件列表可单文件操作
5. （可选）在 **脚本** 页编写 Go 脚本实现过滤、重命名与自动化钩子
6. （可选）在 **日志** 页查看运行日志，在 **教程** 页阅读分级使用教程
...
├── img/         # 品牌视觉素材（图标 / Logo / 插图 / 宣传材料）
├── tmp/         # 临时文件（不入库）
└── AGENTS.md     # AI 编程代理速览指南
```
