...
- **媒体下载**：粘贴消息链接批量下载，多线程 + 断点续传（与 tdl CLI 同源引擎）
- **任务管理**：实时进度、速度展示，支持暂停 / 恢复 / 取消
- **脚本引擎**（Yaegi，Go 语法）：
  - `Filter` / `Rename` —— 按条件跳过文件、自定义文件名
  - `OnTaskStart` / `OnFileDone` / `OnTaskDone` —— 任务生命周期钩子
- **代理支持**：SOCKS5 / HTTP 代理
- **简约界面**：亮 / 暗双主题（可跟随系统），单一二进制免安装

## 界面预览

> （截图占位：登录页 / 下载页 / 任务页 / 脚本页 / 设置页）
...
环境要求：Go 1.23+、Node.js 20+、pnpm、[Wails CLI v2](https://wails.io/docs/gettingstarted/installation)。
...
2. 在 **下载** 页粘贴消息链接（每行一条），选择保存目录，点击创建
3. 在 **任务** 页查看实时进度，可暂停 / 恢复（断点续传）/ 取消
4. （可选）在 **脚本** 页编写 Go 脚本实现过滤、重命名与自动化钩子
